...
def make_encoder(opt, embeddings, morph_embeddings=None, encoder_type=None):
    """
    ...
    if encoder_type == "transformer":
        ...
        if opt.use_dgl:
            return GCNEncoder_DGL(embeddings,
                                  opt.gcn_num_inputs,
                                  opt.gcn_num_units,
                                  opt.gcn_num_labels,
                                  opt.gcn_num_layers,
                                  ...
                                  opt.gcn_use_glus,
                                  morph_embeddings,
                                  use_birnn=opt.brnn, rnn_layer=opt.enc_layers)
        else:
            return GCNEncoder(embeddings,
                              ...
                              opt.gcn_use_gates,
                              opt.gcn_use_glus,
                              morph_embeddings, use_birnn=opt.brnn, rnn_layer=opt.enc_layers)
    else:
        # "rnn" or "brnn"
...
def make_decoder(opt, embeddings):
    ...
    if opt.decoder_type == "transformer":
        ...
        #                           opt.dropout, embeddings)
        return GPTD(opt.dec_layers, opt.rnn_size,
                                  opt.global_attention, opt.copy_attn,
                                  opt.dropout, embeddings)
...
def make_base_model(model_opt, fields, gpu, checkpoint=None):
    ...
    if model_opt.model_type == "text":
        ...
        if 'morph' in fields and hasattr(fields["morph"], 'vocab'):
                ...
                encoder2 = make_encoder(model_opt, src_embeddings2, morph_embeddings, encoder_type='rnn') if not model_opt.encoder2_type == 'none' else None
            # else:
            ...
            encoder = make_encoder(model_opt, src_embeddings, encoder_type=model_opt.encoder_type)  # gcn features must go here
            if model_opt.encoder2_type == 'none':
                encoder2 = None
            else:
                if model_opt.encoder2_type == 'gcn':
                    encoder2 = make_encoder(model_opt, src_embeddings, encoder_type='gcn')  # gcn features must go here
                elif model_opt.encoder2_type == 'rnn':
                    encoder2 = make_encoder(model_opt, src_embeddings2, encoder_type='rnn')
                else:
